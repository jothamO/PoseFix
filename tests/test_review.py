...
def test_pass_review():
    checks = {
        ...
        "pose_target_adherence": 0.91,
        "anatomical_plausibility": 0.96,
        "hand_quality": 0.94,
        "clothing_retention": 0.95,
...
def test_hard_violation_rejects():
    assert decide_review(
        {"identity_retention": 0.99},
        ["extra_limb"],
    )["decision"] == "REJECT"


def test_under_applied_pose_target_retries():
    checks = {
        "identity_retention": 0.97,
        "pose_target_adherence": 0.62,
    ...
    review = decide_review(checks, [])

    assert review["decision"] == "RETRY"
    assert review["retry_recommendation"] == {
